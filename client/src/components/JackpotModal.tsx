    ...
    <Modal open={open} onClose={phase === 'spinning' ? () => {} : onClose} title="Jackpot Spin">
      <div className="space-y-6 py-2">
        <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground">{variantName} · {formatCents(basePrice)}</p>
        </div>
          ...
          {phase === 'result' && spinResult && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-background/95 backdrop-blur-sm border border-border rounded-2xl p-5 text-center shadow-xl mx-4 space-y-3 w-full">
                <p className="text-4xl font-black">{spinResult.multiplierPct === 0 ? '🍀' : spinResult.multiplierPct === 200 ? '😬' : spinResult.multiplierPct < 100 ? '🎉' : '😅'}</p>
                <p className="text-2xl font-bold">{spinResult.multiplierPct === 0 ? 'Gratis!' : `${spinResult.multiplierPct}%`}</p>
                <div className="text-sm text-muted-foreground">
                    Bezahlt: <span className={cn("font-semibold", spinResult.multiplierPct <= 100 ? "text-green-500" : "text-red-500")}>{formatCents(spinResult.pricePaid)}</span>
                </div>
                <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium">Fertig</button>
              </div>
            </div>
          )}
        </div>
        ...
        {phase === 'spinning' && (
           <button disabled className="w-full py-4 rounded-2xl bg-muted text-muted-foreground font-bold text-lg cursor-not-allowed">
             Glück am Rad...
           </button>
        )}
      </div>
