...
function RankMedal({ rank }: { rank: number }) {
  ...
  return (
    <span className="w-7 text-center text-sm font-semibold text-muted-foreground">{rank}</span>
  );
}
        ...
        <div className="rounded-2xl border border-border overflow-hidden">
          {isLoading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="flex items-center gap-3 px-4 py-3 bg-card border-b border-border last:border-0 animate-pulse"
              >
