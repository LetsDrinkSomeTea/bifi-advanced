...
export function AdminLayout({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  const tabs: TabItem[] = [
    ...
    ...(isAdmin ? [{ id: 'audit-log', label: 'Audit-Log', href: '/admin/audit-log' }] : []),
  ];

  const activeTab = tabs.find((tab) => location.startsWith(tab.href))?.id ?? 'users';
  ...
  return (
    <Layout>
      <div className="px-4 py-4 max-w-lg mx-auto space-y-4">
        ...
        <TabContent activeId={activeTab} items={tabs} onTabChange={handleTabChange}>
          {children}
        </TabContent>
      </div>
    </Layout>
