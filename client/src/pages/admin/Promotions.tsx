...
function PromotionModal({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (promotion) {
      update({ id: promotion.id, ...body }, { onSuccess: onClose, onError: (err) => setError(err instanceof Error ? err.message : 'Fehler') })
    } else {
      create(body, { onSuccess: onClose, onError: (err) => setError(err instanceof Error ? err.message : 'Fehler') })
    }
  }
    ...
    <Modal open={open} onClose={onClose} title={promotion ? 'Rabatt bearbeiten' : 'Neuer Rabatt'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{type === 'percent' ? 'Rabatt (%)' : 'Neuer Preis (€)'}</label>
            <input
              type="number"
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => { setTargetBuyableId(e.target.value); setTargetVariantId('') }}
            className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
          >
    ...
    <AdminLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => { setSelectedPromo(null); setModalOpen(true) }}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={cn(
                        ...
                        p.discountFixedCents != null ? "bg-orange-500/10 text-orange-500" : "bg-green-500/10 text-green-500"
                      )}>
                        {p.discountFixedCents != null ? `${formatCents(p.discountFixedCents)} Fix` : `-${p.discountPercent}%`}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className={cn(
                        ...
                        p.isActive ? "text-orange-500 hover:bg-orange-500/10" : "text-green-500 hover:bg-green-500/10"
                      )}
                    ...
                    <button
                      onClick={() => { if(confirm('Löschen?')) remove(p.id) }}
      ...
      <PromotionModal
        open={modalOpen}
        onClose={() => { setModalOpen(false); setSelectedPromo(null) }}
