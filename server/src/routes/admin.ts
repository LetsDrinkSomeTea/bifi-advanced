    ...
    .where(
      parsed
        ? or(
            ...
            and(
              eq(transactions.createdAt, new Date(parsed.t)),
              lt(transactions.id, parsed.id),
            ),
          )
        : undefined,
    )
...
router.delete("/users/:id", requireRole("admin"), async (c) => {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .delete(activityFeed)
      .where(
        or(eq(activityFeed.userId, id), eq(activityFeed.targetUserId, id)),
      );
    await tx
      .delete(donationContributions)
      .where(eq(donationContributions.userId, id));
    await tx
      .delete(userFriendships)
      .where(
        or(
          eq(userFriendships.requesterId, id),
          eq(userFriendships.addresseeId, id),
        ),
      );
    await tx
      .delete(nudges)
      .where(or(eq(nudges.senderId, id), eq(nudges.recipientId, id)));
    await tx
      .delete(prostVouchers)
      .where(
        or(eq(prostVouchers.fromUserId, id), eq(prostVouchers.toUserId, id)),
      );
