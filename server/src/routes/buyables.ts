...
router.get('/', requireAuth, async (c) => {
  ...
  const showAll = c.req.query('all') === 'true' && (user.role === 'admin' || user.role === 'moderator')
...
router.post('/', requireAuth, requireRole('moderator'), zValidator('json', CreateBuyableSchema), async (c) => {
  const body = c.req.valid('json')
  const actor = c.get('user')
  const ip = c.req.header('x-forwarded-for')?.split(',')[0]?.trim() ?? null

  const { buyable: created, variant } = await db.transaction(async (tx) => {
    const [buyable] = await tx.insert(buyables).values({
      name: body.name,
...
router.put('/:id', requireAuth, requireRole('moderator'), zValidator('json', UpdateBuyableSchema), async (c) => {
...
router.delete('/:id', requireAuth, requireRole('admin'), async (c) => {
  ...
  await db.update(buyables).set({ isActive: false, updatedAt: new Date() }).where(eq(buyables.id, id))
...
router.post('/:id/variants', requireAuth, requireRole('moderator'), zValidator('json', CreateVariantSchema), async (c) => {
  const { id } = c.req.param()
  const body = c.req.valid('json')

  const [parent] = await db.select().from(buyables).where(eq(buyables.id, id))
  if (!parent || !parent.isActive) return c.json({ error: 'Buyable not found', code: 'NOT_FOUND' }, 404)
...
router.put('/:id/variants/:variantId', requireAuth, requireRole('moderator'), zValidator('json', UpdateVariantSchema), async (c) => {
