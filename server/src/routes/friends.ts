...
router.post('/:userId/request', requireAuth, async (c) => {
  ...
  if (userId === user.id) return c.json({ error: 'Cannot friend yourself', code: 'SELF_FRIEND' }, 400)
  ...
  // Create new request
  const [targetUser] = await db.select().from(users).where(and(eq(users.id, userId), eq(users.isActive, true)))
