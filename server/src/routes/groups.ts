...
router.get('/', requireAuth, async (c) => {
  ...
  const rows = await db
    ...
    .from(groups)
    .innerJoin(groupMembers, and(eq(groupMembers.groupId, groups.id), eq(groupMembers.userId, user.id), isNull(groupMembers.leftAt)))
...
router.post('/join', requireAuth, joinRateLimit, zValidator('json', z.object({ inviteCode: z.string() })), async (c) => {
  ...
  if (existing) {
    if (!existing.leftAt) return c.json({ error: 'Already a member', code: 'ALREADY_MEMBER' }, 409)
    await db
      .update(groupMembers)
...
router.post('/:id/leave', requireAuth, async (c) => {
  ...
  if (membership.role === 'owner') {
    ...
    if (count <= 1) {
      ...
      await db.update(groupMembers).set({ leftAt: new Date() }).where(and(eq(groupMembers.groupId, id), isNull(groupMembers.leftAt)))
  ...
  await db.update(groupMembers).set({ leftAt: new Date() }).where(and(eq(groupMembers.groupId, id), eq(groupMembers.userId, user.id)))
...
router.delete('/:id/members/:userId', requireAuth, async (c) => {
  ...
  await db.update(groupMembers).set({ leftAt: new Date() }).where(and(eq(groupMembers.groupId, id), eq(groupMembers.userId, userId)))
  return c.body(null, 204)
})
...
router.delete('/:id', requireAuth, async (c) => {
  ...
  const [[myMembership], [group]] = await Promise.all([
    db.select().from(groupMembers).where(and(eq(groupMembers.groupId, id), eq(groupMembers.userId, self.id), isNull(groupMembers.leftAt))),
    db.select({ id: groups.id, name: groups.name }).from(groups).where(eq(groups.id, id)),
  ])
  if (myMembership?.role !== 'owner') return c.json({ error: 'Forbidden', code: 'FORBIDDEN' }, 403)
